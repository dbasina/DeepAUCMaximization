...
class CheXpert(Dataset):
    ...
    def image_augmentation(self, image):
        img_aug = transforms.Compose([transforms.RandomAffine(degrees=(-15, 15), translate=(0.05, 0.05), scale=(0.95, 1.05), fill=128)]) # pytorch 3.7: fillcolor --> fill
        image = img_aug(image)
        return image
    ...
    def __getitem__(self, idx):
        ...
        # Use PIL and force RGB (replicates grayscale to 3 channels if needed)
        img = Image.open(path).convert("RGB")

        if self.mode == "train":
            if self.transforms is None:
                # fallback default train pipeline (change size if you want 224/320)
                ...
                img = self.transforms(img)  # should already end with ToTensor+Normalize
        else:
            if self.transforms is None:
        ...
        # labels
        y = np.array(self.targets[idx], dtype=np.float32).reshape(-1)
